...
argument-hint: "[config-file|model-id|source-card-id]"
...
```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/create_question.py" --name "每日访客数" --model-id 5952 --config-file question_config.json --collection 485 --verify
```
...
- Use MBQL patterns from `${CLAUDE_SKILL_DIR}/../kmb-metabase/references/mbql-best-practices.md` when the query is non-trivial.
