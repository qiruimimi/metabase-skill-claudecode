#!/usr/bin/env python3
"""
创建 Metabase Collection 的辅助脚本。
提供命令行接口封装 API 调用，处理常见边界情况（如已存在）。
"""
...
API_KEY = None  # 从环境变量或命令行获取


def get_headers():
    """获取请求头"""
    if not API_KEY:
        raise ValueError("API_KEY 未设置，请通过 --api-key 参数或 KMB_API_KEY 环境变量设置")
    return {
        "X-API-Key": API_KEY,
...
def main():
    ...
    if not API_KEY:
        print("❌ 错误: 请提供 API Key (通过 --api-key 参数或 KMB_API_KEY 环境变量)", file=sys.stderr)
        sys.exit(1)
